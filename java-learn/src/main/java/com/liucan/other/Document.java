...
public class Document {
     ...
     *
     * 12.多态
     *   a.动态绑定，相对于静态绑定在编译的时候就决定调用哪个函数，而是在运行的时候才决定实例真正指向哪个对象
     *   b.具体实现就是每个类有个虚方法表，里面保存了虚方法的的地址，如果没有重写，则是指向父类的，重写了则指向自己
     *   c.成员变量是不支持多态的，因为不需要，方法是类提供给外部调用的，而变量则是内部使用，不对外公开，没有必要支持
     *
     * 13.反射和动态代理？
     *
     * 14.外部类，只能用public和default，因为外部类的上一级就是包，权限就只有在包内和包外，没有其他的
     */
}
